...
// client/src/components-project.js
var capturePhaseOf = (state, lastError) => {
  switch (state) {
    case "opening":
      return { text: "正在打开 WebVPN 窗口…", tone: "busy" };
    case "waiting-login":
      return { text: "请在 WebVPN 窗口完成登录，再点击「我已登录」", tone: "waiting" };
    case "ready":
      return { text: "正在打开出版社页面…", tone: "busy" };
    case "navigating":
      return { text: "正在加载出版社页面…", tone: "busy" };
    case "waiting-download":
      return { text: "出版社页面已打开，请点击「下载 PDF / SI」按钮", tone: "waiting" };
    case "downloading":
      return { text: "正在下载文件…", tone: "busy" };
    case "uploading":
      return { text: "文件已下载，正在归档到课题…", tone: "busy" };
    case "expired":
      return { text: "捕获任务已过期，请重新点击文献按钮", tone: "error" };
    case "error":
      return { text: lastError ? `捕获失败：${lastError}` : "捕获失败，请重试", tone: "error" };
    default:
      return { text: "正在准备捕获…", tone: "busy" };
  }
};
  ...
  (0, import_react8.useEffect)(() => {
    ...
    const poll = async () => {
      try {
        ...
        setCaptureHint((current) => current?.taskId === taskId ? { ...current, phase: capturePhaseOf(status.state, status.lastError) } : current);
      } catch {
      }
