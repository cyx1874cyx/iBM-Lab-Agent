...
function NmrRegistry({ rows = [] }) {
  ...
  return h("div", { className: "sw-struct" }, rows.map((row) => {
    const compound = row.compound || { name: row.name, smiles: void 0 };
    return h(
      "article",
      { className: "sw-struct-card", key: row.id, style: { minWidth: 190 } },
      compound.smiles ? h(StructureCard, { entry: compound, onClick: () => {
      } }) : h("div", { style: { height: 84, display: "grid", placeItems: "center", color: "#8aa7c6" } }, "结构待补充"),
...
function PlotRegistry({ projectId, call }) {
  const [state, setState] = (0, import_react4.useState)({ loading: true, list: [], error: "" });
  const [newTopic, setNewTopic] = (0, import_react4.useState)("");
  const [newDate, setNewDate] = (0, import_react4.useState)("");
  const [editId, setEditId] = (0, import_react4.useState)(null);
  const [editTopic, setEditTopic] = (0, import_react4.useState)("");
  const [editDate, setEditDate] = (0, import_react4.useState)("");
  const [busy, setBusy] = (0, import_react4.useState)({});
  const [toast, setToast] = (0, import_react4.useState)("");
  (0, import_react4.useEffect)(() => {
    setState((s) => ({ ...s, loading: true, error: "" }));
    call("plot_records_list", { request: { projectId } }).then((result) => setState({ loading: false, list: result.records || [], error: "" })).catch((reason) => setState((s) => ({ ...s, loading: false, list: s.list || [], error: reason.message })));
  }, [call, projectId]);
  const withBusy = (key, fn) => {
    if (busy[key]) return;
    setBusy((s) => ({ ...s, [key]: true }));
  ...
  const add = () => withBusy("add", async () => {
    ...
    await call("plot_records_create", { request: { id: `plot-${Date.now().toString(36)}`, projectId, topic, date: newDate || void 0, source: "manual" } });
    setNewTopic("");
    setNewDate("");
  ...
  return h(
    "div",
    { className: "ib-card", style: { marginTop: 14 } },
    ...
    state.error ? h("div", { className: "ib-error" }, state.error) : null,
    state.loading ? h("div", { className: "ib-empty" }, "加载中…") : null,
    state.list.length ? h("div", { className: "ib-rows" }, state.list.map((row) => editId === row.id ? h(
      "div",
      { className: "ib-row", key: row.id, style: { gap: 8 } },
...
function PdfViewerFrame({ row, notify }) {
  ...
  (0, import_react4.useEffect)(() => {
    ...
    const postOpen = () => {
      try {
        iframeRef.current?.contentWindow?.postMessage({ type: "open", bundleId, kind: documentKind, page: pageNumber, quote }, "*");
      } catch {
      }
    };
...
function KetcherEditorModal({ entry, onSave, onCancel }) {
  ...
  return h(
    ...
    h(
      ...
      status === "timeout" ? h(
        "div",
        { style: { flex: 1, padding: 16, background: "#fff", display: "flex", flexDirection: "column", gap: 10, minHeight: 0 } },
  ...
  return h(
    ...
    h(
      ...
      lockBlockers.length ? h(
        "div",
        { className: "ib-error", style: { marginTop: 8, border: "1px solid rgba(255,137,137,.4)", padding: "10px 12px", borderRadius: 10 } },
        h("b", null, "锁定被阻断："),
        lockBlockers.map((blocker, index) => h(
          ...
          (blocker.evidenceIds || []).length ? h("span", { style: { marginLeft: 6, color: "#8aa7c6" } }, `事实：${blocker.evidenceIds.join("、")}`) : null
        ))
      ) : null,
      h(
        "div",
    ...
    selectedStep && detail ? h(
      ...
      batchList.filter((row) => row.stepId === selectedStep.id).length ? h(
        "div",
        { className: "sw04-batchbar", style: { borderColor: "rgba(112,157,211,.3)", background: "rgba(1,20,45,.4)" } },
        h("b", null, "审核批次"),
    ...
    dualPanel ? h("div", { className: "sw-struct-edit" }, h(
      "div",
      { style: { width: "min(820px,96vw)", maxHeight: "84vh", overflowY: "auto", display: "grid", gap: 10, background: "#011e3f", border: "1px solid rgba(140,181,229,.42)", borderRadius: 14, padding: 16, color: "#cfe4fb", fontSize: 11, lineHeight: 1.6 } },
      h(
        "div",
      ...
      dualPanel.results.length ? dualPanel.results.map((item) => {
        ...
        const srcRow = h(
          "div",
          { style: { display: "flex", gap: 14, flexWrap: "wrap", fontSize: 9.5, color: "#9db8d6" } },
...
function Project({ call, project, onBack, onDelete, onStartChat, onOpenSearch }) {
  ...
  const load = (0, import_react7.useCallback)(async () => {
    try {
      ...
      setDraft(data2.memory?.markdown || "");
    } catch (reason) {
