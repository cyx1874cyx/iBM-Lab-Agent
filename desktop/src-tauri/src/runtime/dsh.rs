...
fn replace_tree(source: &Path, target: &Path) -> Result<(), RuntimeError> {
    if target.exists() {
        fs::remove_dir_all(target).map_err(|error| RuntimeError::new(format!("Cannot remove stale managed data {}: {error}", target.display())))?;
    }
...
pub fn bootstrap_user_data(layout: &RuntimeLayout, logger: &AppLogger) -> Result<(), RuntimeError> {
  ...
  "dsh": { "profile": { "bundles": ["@deepseek-ai/dsh-base", "@deepseek-ai/dsh-web-app", "dsh-lab-agent"] } }
    ...
    let package_manifest = fs::read_to_string(bundled_plugin.join("package.json"))
        .map_err(|error| RuntimeError::new(format!("Cannot read bundled plugin manifest: {error}")))?;
    ...
    let desired_state = format!("desktop={}\nmanifest={}\nvendor={}\nrequirements={}\n", env!("CARGO_PKG_VERSION"), package_manifest, vendor_lock, requirements);
    ...
    if current_state != desired_state {
        ...
        replace_tree(&bundled_plugin, &profile_dir.join("node_modules").join("dsh-lab-agent"))?;
        replace_tree(&plugin.join("presets").join("lab-research"), &layout.dsh_home.join(".agent-presets").join("lab-research"))?;
        ...
        fs::copy(plugin.join("vendor.lock.json"), lab_home.join("vendor.lock.json"))
            .map_err(|error| RuntimeError::new(format!("Cannot copy vendor.lock.json: {error}")))?;
        fs::copy(bundled_plugin.join("python").join("requirements.lock"), lab_home.join("requirements.lock"))
        ...
        fs::write(marker, desired_state).map_err(|error| RuntimeError::new(format!("Cannot write bootstrap marker: {error}")))?;
        let legacy_marker = lab_home.join(".desktop-bootstrap-v1");
        if legacy_marker.exists() { let _ = fs::remove_file(legacy_marker); }
    }
    Ok(())
}
...
mod tests {
    ...
    fn materializes_required_lab_data_and_refreshes_managed_trees() {
        ...
        let sandbox = std::env::temp_dir().join(format!("ibm-lab-bootstrap-{}-{nonce}", std::process::id()));
        let resources = sandbox.join("resources");
        let plugin = resources.join("plugin");
        let bundled_plugin = plugin.join("dsh-lab-agent");
        write_file(&resources.join("node").join("node.exe"), "node");
        write_file(&resources.join("dsh").join("node_modules").join("@deepseek-ai").join("dsh").join("lib").join("bin.js"), "dsh");
        write_file(&bundled_plugin.join("package.json"), r#"{"version":"1.0.0"}"#);
        write_file(&bundled_plugin.join("vendor").join("nature-skills").join("skills").join("sample").join("SKILL.md"), "v1");
        write_file(&bundled_plugin.join("python").join("requirements.lock"), "example==1.0\n");
        write_file(&plugin.join("presets").join("lab-research").join("agent.cordis.yml"), "preset-v1");
        ...
        let lab_home = layout.dsh_home.join("lab-agent");
        let profile_plugin = layout.dsh_home.join("profiles").join("ibm-lab").join("node_modules").join("dsh-lab-agent");
        assert!(lab_home.join("vendor").join("nature-skills").join("skills").join("sample").join("SKILL.md").exists());
        assert_eq!(fs::read_to_string(lab_home.join("requirements.lock")).unwrap(), "example==1.0\n");
        ...
        write_file(&bundled_plugin.join("package.json"), r#"{"version":"2.0.0"}"#);
        write_file(&bundled_plugin.join("python").join("requirements.lock"), "example==2.0\n");
        ...
        assert_eq!(fs::read_to_string(lab_home.join("requirements.lock")).unwrap(), "example==2.0\n");
        assert_eq!(fs::read_to_string(lab_home.join("vendor.lock.json")).unwrap(), r#"{"pinned":"v2"}"#);
