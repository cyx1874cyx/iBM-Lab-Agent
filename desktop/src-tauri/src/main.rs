...
use runtime::{AppConfig, RuntimeManager, RuntimeDeps, RuntimeStatus};
...
fn open_in_edge(url: String) -> Result<(), String> {
    ...
    let candidates = [
        ...
        std::env::var_os("LOCALAPPDATA")
            .map(|base| std::path::PathBuf::from(base).join(r"Microsoft\Edge\Application\msedge.exe"))
            .unwrap_or_default(),
    ];
    ...
    let _ = child.kill(); // 立即回收句柄：msedge.exe 会 fork 出真实进程
    Ok(())
}
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![restart_runtime, open_logs, runtime_status, runtime_deps, load_config, save_config, open_in_edge])
