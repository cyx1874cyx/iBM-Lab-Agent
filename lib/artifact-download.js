/**
 * Same-origin binary download and PDF preview endpoint for research artifacts.
...
export function createArtifactDownloadHandler(tasks, { renderPreview, characterization } = {}) {
	return async (req, res) => {
		...
		const officePreview = preview && (kind === "report" || kind === "ppt");
		const pdfPreview = preview && (kind === "pdf" || kind === "si");
		try {
			let file;
			if (kind === "report") {
				...
				file = await tasks.readingReportFile(reportId, preview ? "docx" : format, { requireApproved: false });
			} else if (kind === "characterization") {
    const taskId=url.searchParams.get("taskId")||""; const projectId=url.searchParams.get("projectId")||""; const slot=url.searchParams.get("slot");
    if(!ID_RE.test(taskId)||!ID_RE.test(projectId)||!["spectrum","report","origin"].includes(slot))throw new Error("invalid characterization artifact");
    file=await characterization().artifactFile(taskId,projectId,slot);
   } else if (kind === "ppt") {
				const reportId = url.searchParams.get("reportId") ?? "";
				if (!ID_RE.test(reportId)) {
					sendError(res, 400, "invalid reportId");
				...
				file = await tasks.presentationFile(reportId, { requireApproved: false });
				...
				if (preview && file.mime !== "application/pdf" && !/\.pdf$/i.test(file.fileName)) {
					sendError(res, 400, "inline preview is only available for PDF files");
					return;
				}
			} else {
			...
			res.writeHead(200, {
				...
				"content-disposition": `${pdfPreview ? "inline" : "attachment"}; filename*=UTF-8''${encodeURIComponent(file.fileName)}`,
			...
			const status = officePreview && /renderer|LibreOffice|soffice/i.test(error.message || "") ? 503 : 404;
			if (officePreview) sendPreviewError(res, status, error.message || "artifact unavailable");
...
export class LabArtifactDownloadService extends Service {
	...
	async [Service.init]() {
		...
		const handler = createArtifactDownloadHandler(this.ctx.labTasks, { renderPreview: (file) => renderer.render(file), characterization: () => this.ctx.get("labCharacterization") });
