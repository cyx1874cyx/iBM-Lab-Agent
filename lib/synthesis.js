...
import { compoundNameKeys, hydrateStepStructures, mergeStepStructures, structureLookup } from "../src/synthesis/structures.js";
...
import { lookupCactus, resolveCompoundDual, resolveCompoundFirst, lookupCactusFirst } from "../src/synthesis/compound-resolve.js";
import {
	assertCasAuthorized,
	prepareCasQuery,
	casLoginEntry,
...
function hydrateRoute(route) {
	...
	for (const compound of route.compounds ?? []) {
		if (compound?.label) {
			for (const key of compoundNameKeys(compound.label)) compoundsByLabel.set(key, compound.smiles);
		}
	}
	const steps = (route.steps ?? []).map((step, index) => {
		...
		hydrated.structures = hydrated.structures.map((row) => {
			if (row.smiles) return row;
			const key = compoundNameKeys(row.name).find((candidate) => compoundsByLabel.has(candidate));
			return key ? { ...row, smiles: compoundsByLabel.get(key), source: "entity" } : row;
		});
		return hydrated;
	});
	return { ...route, steps };
...
/** 新写入 Evidence 的页码必须可无歧义映射到 PDF 页；旧数据读取仍保持兼容。 */
export function normalizeEvidencePage(value, { required = false } = {}) {
	if (value === undefined || value === null || String(value).trim() === "") {
		if (required) throw new Error("evidence page required; use a plain PDF page such as 5 or S12");
		return undefined;
	}
	const page = String(value).trim().toUpperCase();
	if (!/^(?:S)?[1-9]\d*$/.test(page)) {
		throw new Error(`invalid evidence page '${String(value)}'; use digits only (for example 5 or S12), and put Scheme/Figure labels in figure`);
	}
	return page;
}
...
export class LabSynthesisService extends Service {
	...
	evidenceRequiresShot(row) {
		// 已归档 PDF/SI 由审核抽屉直接展示并由用户核对，不再强制派生截图。
	...
	/**
	 * 删除一条未锁定路线，并级联清理只属于该路线的事实证据与审核批次。
	 * 有后续版本时拒绝删除，避免 parentRouteId 形成悬空版本链；锁定版本始终只读。
	 */
	async deleteRoute(id) {
		const route = this.getRoute(id);
		this.assertWritable(route, "delete route");
		const children = this.listRoutes().filter((row) => row.parentRouteId === id);
		if (children.length) {
			throw new Error(`route '${id}' has ${children.length} child revision(s); delete newer revisions first`);
		}
		const evidenceIds = [...this.table("evidence").keys()]
			.filter((key) => this.table("evidence").get(key)?.routeId === id);
		const batchIds = [...this.table("reviewBatches").keys()]
			.filter((key) => this.table("reviewBatches").get(key)?.routeId === id);
		for (const key of evidenceIds) await this.table("evidence").delete(key);
		for (const key of batchIds) await this.table("reviewBatches").delete(key);
		await this.table("routes").delete(id);
		return { id, deleted: true, evidenceDeleted: evidenceIds.length, reviewBatchesDeleted: batchIds.length };
	}
	...
	async resolveStepCompoundsDual(routeId, stepKeyOrId, deps = {}) {
		...
		for (const name of missing) {
			const result = await this.resolveCompoundFirst(name, deps);
	...
	/**
	 * 登记由 Agent 给出的新产物结构候选。候选立即进入路线用于预览，但同时创建
	 * pending Evidence；只有事实核验确认/修正后才标记为人工决定。
	 */
	async addStructureCandidate({ routeId, stepId, name, smiles, role = "product", method, sourceName, sourceType, sourceTier, sourceKind, bundleId, documentId, page, figure, bbox, excerpt, doi, confidence = "unknown" }) {
		const route = this.getRoute(routeId);
		this.assertWritable(route, "add structure candidate");
		if (!["literature-inference", "visual-extraction"].includes(method)) throw new Error("structure candidate method must be literature-inference or visual-extraction");
		const cleanName = String(name ?? "").replace(/\s+/g, " ").trim();
		const cleanSmiles = String(smiles ?? "").trim();
		if (!cleanName || !cleanSmiles) throw new Error("structure candidate name and smiles required");
		if (!sourceName) throw new Error("structure candidate sourceName required");
		if (!(bundleId || documentId) || page === undefined || page === null || page === "") {
			throw new Error("structure candidate requires archived bundleId/documentId and page for human verification");
		}
		const normalizedPage = normalizeEvidencePage(page, { required: true });
		const key = String(stepId);
		const targetIndex = route.steps.findIndex((step) => step.id === key || String(step.step) === key);
		if (targetIndex < 0) throw new Error(`step '${key}' not found in route '${routeId}'`);
		const step = route.steps[targetIndex];
		const now = new Date().toISOString();
		const evidenceId = `${safeId("ev-structure", `${routeId}-${step.id ?? step.step}`)}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`;
		const evidence = synthesisEvidenceSchema.parse({
			id: evidenceId,
			routeId,
			stepId: step.id ?? `s${step.step}`,
			supportsField: `structure.${role}.${cleanName}`,
			sourceType: sourceType ?? (method === "visual-extraction" ? "paper-main" : "model-inference"),
			sourceTier: sourceTier ?? (method === "visual-extraction" ? 2 : 5),
			sourceKind,
			sourceName,
			doi,
			bundleId,
			...
			figure,
			bbox,
			excerpt: excerpt ?? cleanSmiles,
			relation: "supports",
			extractionMethod: method === "visual-extraction" ? "vlm" : "model",
			confidence,
			reviewStatus: "pending",
			originalExtract: cleanSmiles,
			structureCandidate: { name: cleanName, smiles: cleanSmiles, role, method },
			createdAt: now,
			updatedAt: now
		});
		const hydrated = hydrateStepStructures(step);
		const mergedStructures = mergeStepStructures({ ...step, structures: hydrated }, [{
			name: cleanName,
			smiles: cleanSmiles,
			role,
			source: method,
			evidenceId,
			verification: { status: "unresolved", sources: [method], checkedAt: now, notes: "Agent 结构候选，等待事实核验" },
			updatedAt: now
		}]);
		const steps = route.steps.map((item, index) => index === targetIndex ? { ...step, structures: mergedStructures } : item);
		const parsedRoute = synthesisRouteSchema.parse({ ...route, steps, updatedAt: now });
		await this.table("routes").put(routeId, parsedRoute);
		await this.table("evidence").put(evidenceId, evidence);
		return { route: hydrateRoute(parsedRoute), evidence };
	}
	 ...
	 */
 async resolveCompoundFirst(identifier, deps = {}) {
  const { lookupPubChemCas } = await import("../src/chemistry/rdkit-pubchem.js");
  let primary;
  const pubchem = query => primary ??= Promise.resolve().then(() => (deps.pubchem || lookupPubChem)(query));
  const casLookup = pubchem(identifier).then(props => (deps.pubchemCas || lookupPubChemCas)(identifier, {cid:props?.cid, fetchImpl:deps.pubchemFetchImpl})).catch(() => null);
  const result = await resolveCompoundFirst(identifier, {pubchem, cactus:deps.cactus || lookupCactusFirst, timeoutMs:deps.timeoutMs});
  if (!result.casNumber) {
   let timer;
   try {
    const cas = await Promise.race([casLookup, new Promise(resolve => { timer=setTimeout(()=>resolve(null), deps.casTimeoutMs ?? 3000); })]);
    if(cas?.casNumber) { result.casNumber=cas.casNumber; result.sources={...result.sources,pubchem:{...result.sources?.pubchem,casNumber:cas.casNumber,casSource:cas.casSource}}; }
  ...
  return result;
	...
	async addStepEvidence({ id, routeId, stepId, stepKey, ...rest }) {
		...
		const normalizedPage = normalizeEvidencePage(rest.page);
		if (normalizedPage !== undefined) rest = { ...rest, page: normalizedPage };
		else if (Object.prototype.hasOwnProperty.call(rest, "page")) {
			const { page: _emptyPage, ...withoutPage } = rest;
			rest = withoutPage;
		}
		const now = new Date().toISOString();
	...
	/** 将结构候选的人工判定同步回路线；普通 Evidence 返回 null。 */
	structureCandidateRouteAfterReview(row, status, correction) {
		const candidate = row?.structureCandidate;
		if (!candidate) return null;
		const route = this.getRoute(row.routeId);
		const targetIndex = route.steps.findIndex((step) => step.id === row.stepId || (row.stepKey !== undefined && step.step === row.stepKey));
		if (targetIndex < 0) throw new Error(`structure candidate step '${row.stepId ?? row.stepKey}' not found`);
		const step = route.steps[targetIndex];
		const hydrated = hydrateStepStructures(step);
		const lookup = structureLookup(hydrated);
		const current = compoundNameKeys(candidate.name).map((key) => lookup[key]).find(Boolean)
			?? hydrated.find((entry) => entry?.name === candidate.name);
		if (current?.evidenceId && current.evidenceId !== row.id) {
			throw new Error("该结构候选已被更新版本替代，请审核最新候选");
		}
		const now = new Date().toISOString();
		const matchesSavedCorrection = !!row.userCorrection
			&& current?.evidenceId === row.id
			&& current?.smiles === row.userCorrection;
		let addition;
		if (status === "rejected") {
			// 仅撤销仍与本条候选或本条人工修正一致的结构；另行修改的结构不回滚。
			if (current?.smiles !== candidate.smiles && !matchesSavedCorrection) return null;
			addition = {
				...current,
				name: candidate.name,
				smiles: undefined,
				...
				evidenceId: row.id,
				verification: { status: "unresolved", sources: [candidate.method, "human-rejected"], checkedAt: now, notes: "候选结构已人工判定为无法确认" },
				updatedAt: now
			};
			...
			addition = {
				...current,
				name: candidate.name,
				smiles: matchesSavedCorrection ? row.userCorrection : candidate.smiles,
				role: candidate.role,
				source: matchesSavedCorrection ? "manual" : candidate.method,
				evidenceId: row.id,
				verification: { status: "manual", sources: [candidate.method, ...(matchesSavedCorrection ? ["ketcher"] : []), "human"], checkedAt: now, notes: matchesSavedCorrection ? "人工修正结构已通过事实核验" : "候选结构已通过事实核验" },
				updatedAt: now
			};
			...
			addition = {
				...current,
				name: candidate.name,
				smiles: String(correction).trim(),
				role: candidate.role,
				source: "manual",
				evidenceId: row.id,
				verification: { status: "manual", sources: [candidate.method, "ketcher", "human"], checkedAt: now, notes: "人工补绘或修正结构" },
				updatedAt: now
			};
		} else {
			return null;
		}
		const structures = mergeStepStructures({ ...step, structures: hydrated }, [addition]);
		const steps = route.steps.map((item, index) => index === targetIndex ? { ...step, structures } : item);
		return synthesisRouteSchema.parse({ ...route, steps, updatedAt: now });
	}
	...
	async reviewEvidence(id, status, { correction } = {}) {
		...
		if (status === "corrected") {
			...
			const candidateRoute = this.structureCandidateRouteAfterReview(row, status, correction);
			if (candidateRoute) await this.table("routes").put(candidateRoute.id, candidateRoute);
...
function pickAgentUpdate(update, row) {
	...
	if (page !== undefined && page !== null) out.page = normalizeEvidencePage(page, { required: true });
