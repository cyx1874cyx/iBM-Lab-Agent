...
export class LabSynthesisService extends Service {
	...
	async createTarget({ id, projectId, name, smiles, formula, entityId, notes }) {
		...
		const target = synthesisTargetSchema.parse({ id, projectId, name, smiles, formula, entityId, notes, createdAt: now, updatedAt: now });
	...
	async createRoute({ id, projectId, targetId, name, steps = [] }) {
		...
		const route = synthesisRouteSchema.parse({ id, projectId, targetId, name, steps, status: "draft", createdAt: now, updatedAt: now });
