...
export class LabCaptureService extends Service {
	...
	reportDesktopWebVpnStatus(value = {}) {
		...
		this.desktopWebVpnStatus = {
			...
			pendingTaskId: typeof value.pendingTaskId === "string" ? value.pendingTaskId : undefined,
	...
	getDesktopWebVpnStatus(now = Date.now()) {
		...
		const stale = !Number.isFinite(ageMs) || ageMs > 10_000;
		return { ...status, ageMs, stale, ready: !stale && status.windowOpen && status.state === "ready" };
	}
	 ...
	 */
	async createCaptureTask({ projectId, bundleId, kind, requestedBy = "user" }) {
	...
	async createAgentCaptureTask({ projectId, bundleId, kind }) {
		const created = await this.createCaptureTask({ projectId, bundleId, kind, requestedBy: "agent" });
	...
	async cancelTask(taskId, reason = "捕获任务已取消") {
		...
		if (["completed", "failed", "expired", "cancelled"].includes(task.status)) return task;
	...
	/**
	 * 保存捕获文件到课题工作区的文献条目目录
	 * （<workspace>/literature/<entryStem>/，正文/SI 都强制 .pdf），
	...
	async saveCapturedFile(task, { buffer, fileName }) {
		...
		const targetName = entryFileName(layout.entryStem, task.kind);
