...
export class LabTemplatesService extends Service {
	...
	async ensureSeed() {
		for (const builtin of BUILTIN_TEMPLATES) {
			...
			try {
				await this.requireTable().put(templateKey(builtin.id, builtin.version), builtin);
			} catch (error) {
	...
	async list() {
		...
		for (const k of [...table.keys()].sort().reverse()) {
			...
			out.push({ id: row.id, version: row.version, name: row.name, status: row.status, pageSize: row.pageSize, updatedAt: row.updatedAt });
		}
		return out;
	}
