...
export class LabTasksService extends Service {
	...
	async selectReadingReportTemplate({ reportId, noteTemplateId, noteTemplateVersion }) {
		...
		if (report.paperCardPath) throw new Error("reading report template cannot change after the report artifact is staged");
		const noteId = noteTemplateId ?? report.noteTemplateSnapshot?.id ?? "note-default";
		const snapshot = await this.ctx.labNoteTemplates.snapshotForTask(noteId, noteTemplateVersion);
		const requirements = this.ctx.labNoteTemplates.toNoteRequirements(snapshot);
		const next = readingReportSchema.parse({
			...report,
			noteTemplateSnapshot: snapshot,
