...
export class LabTasksService extends Service {
	...
	/** 记录课题 ↔ Harness 会话/工作区绑定（创建课题后自动开对话时调用）。 */
	async bindProjectSession({ projectId, sessionId, workspaceId }) {
		this.requireProject(projectId);
		const row = projectSessionSchema.parse({
			projectId,
			sessionId,
			workspaceId,
	...
	getProjectBySession(sessionId) {
		for (const key of this.table("sessions").keys()) {
			...
			if (row.sessionId === sessionId) {
				...
				if (project !== undefined) return { project, sessionId: row.sessionId, workspaceId: row.workspaceId };
			}
		}
		return undefined;
	}
