...
export class LabTasksService extends Service {
	...
	async searchLiterature({ projectId, query, sources, limit, sort, yearFrom, runId, mailto, model, sessionId }) {
		this.requireProject(projectId);
	...
	searchRunRis(runId) {
		...
		for (const r of results) {
			...
			if (r.source && r.source !== "openalex") lines.push(`JO  - ${r.source}`);
			lines.push("ER  -");
			lines.push("");
		}
	...
	async readingReportOverview(reportId) {
		...
		const shortCitation = report.shortCitation || bundle?.title || report.id;
	...
	/** 精读报告下载：默认返回 paper-card Markdown 文本；format="docx" 时返回由该
	 *  Markdown 生成的 .docx（base64）。Word 版与 .md 版同源，均为本模块纯 JS 转换。 */
	async readingReportDownload(reportId, format = "md") {
		...
		const base = `${reportId}`;
		if (format === "docx") {
			const buffer = await markdownToDocx(content, { title: report.titleZh || report.shortCitation || reportId });
			return {
				fileName: `${base}.docx`,
		...
		return { fileName: `${base}.md`, mime: "text/markdown;charset=utf-8", text: content, format: "md" };
	}
	...
	/** 文献汇报 PPT 下载：找到该 report 最新的含 pptx 的 run，以 base64 返回。 */
	...
	async createReadingReport({ projectId, bundleId, goalProfileId, goalProfileVersion, noteTemplateId, noteTemplateVersion, reportId, model, shortCitation, titleZh, summary }) {
		...
		const report = readingReportSchema.parse({
			...
			status: "pending",
			shortCitation: shortCitation ?? bundle.title ?? undefined,
			titleZh,
			summary,
			createdAt: now,
			updatedAt: now
	...
	async completeReadingReport({ reportId, paperCardPath, locatorMode, model, shortCitation, titleZh, summary }) {
		...
		if (shortCitation !== undefined) patch.shortCitation = shortCitation;
	...
	/** createPresentation：门禁——仅审计通过的报告可进入 PPT 阶段；显式 skipAudit 可跳过门禁（调用方需能解释，provenance 记录）。 */
	async createPresentation({ projectId, reportId, templateId, templateVersion, runId, model, skipAudit = false }) {
		this.requireProject(projectId);
		const report = this.table("reports").get(reportId);
		if (report === undefined) throw new Error(`reading report '${reportId}' not found`);
		if (report.audit?.ok !== true && !skipAudit) {
