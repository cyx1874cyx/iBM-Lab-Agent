...
export class LabRemoteService extends TypertRemoteService {
	static inject = ["labVersions", "labGoals", "labNoteTemplates", "labTemplates", "labTasks", "labLiterature", "labChemistry", "labNmr", "labSynthesis", "labPython", "labConvert"];
	initializers;
	...
	async projects_workspace(request) {
		...
		const searches = tasks.listSearchRuns(projectId).map((run) => {
			...
			return { ...run, results: (run.results ?? []).map((paper) => {
				...
				return files ? { ...paper, localPdfUrl: files.pdfUrl || undefined, localSiUrl: files.siUrl || undefined } : paper;
			}) };
		});
		return cleanJson({
			project,
	...
	buildPaperFileIndex(tasks, projectId) {
		...
		const entry = (doi) => {
			...
			if (!key) return null;
			if (!filesByDoi.has(key)) filesByDoi.set(key, { pdfUrl: "", siUrl: "" });
		...
		for (const bundle of tasks.listBundles(projectId)) {
			...
			if (!target) continue;
			if (bundle.pdfPath) target.pdfUrl = `/api/lab-artifacts?kind=pdf&bundleId=${encodeURIComponent(bundle.id)}`;
			if (bundle.siPath) target.siUrl = `/api/lab-artifacts?kind=si&bundleId=${encodeURIComponent(bundle.id)}`;
		}
		try {
