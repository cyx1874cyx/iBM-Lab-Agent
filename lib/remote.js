...
export class LabRemoteService extends TypertRemoteService {
	static inject = ["labVersions", "labGoals", "labNoteTemplates", "labExperimentPlanTemplates", "labPlotRecords", "labTemplates", "labTasks", "labLiterature", "labChemistry", "labNmr", "labSynthesis", "labPython", "labConvert", "labCapture"];
	initializers;
	...
	async tasks_search_delete(request) {
		return cleanJson({ result: await this.require("labTasks").deleteSearchRun(request.runId, request.projectId) });
	}
	...
	buildPaperFileIndex(tasks, projectId) {
		...
		for (const bundle of tasks.listBundles(projectId)) {
			...
			if (bundle.siPath) {
				...
				target.siIsPdf = /\.pdf$/i.test(bundle.siPath);
			}
		}
		try {
	...
	async tasks_report_delete(request) {
		return cleanJson({ result: await this.require("labTasks").deleteReadingReport(request.reportId, request.projectId) });
	}
	...
	// ── labNmr（NMR 工作流） ────────────────────────────────────────────────
 async characterization_list(request) {return cleanJson({tasks:this.require("labCharacterization").list(request.projectId)});}
 async characterization_submit(request) {return cleanJson(await this.require("labCharacterization").submit(request));}
 async characterization_retry(request) {return cleanJson(await this.require("labCharacterization").retry(request.taskId,request.projectId));}
 async characterization_dispatch_failed(request) {return cleanJson({task:await this.require("labCharacterization").update(request.taskId,request.projectId,{attempt:request.attempt,status:"failed",error:request.error})});}
	...
	async synth_route_delete(request) {
		return cleanJson({ result: await this.require("labSynthesis").deleteRoute(request.id) });
	}
	...
	// ── labExperimentPlanTemplates（实验计划模板） ─────────────────────────
	async experiment_plan_templates_list() { return cleanJson({ templates: await this.require("labExperimentPlanTemplates").list() }); }
	async experiment_plan_templates_resolve(request) { return cleanJson({ template: await this.require("labExperimentPlanTemplates").resolve(request.id, request.version) ?? null }); }
	async experiment_plan_templates_create(request) { return cleanJson({ template: await this.require("labExperimentPlanTemplates").create(request.id, request.fields) }); }
	async experiment_plan_templates_update(request) { return cleanJson({ template: await this.require("labExperimentPlanTemplates").update(request.id, request.fields) }); }
	async experiment_plan_templates_copy(request) { return cleanJson({ template: await this.require("labExperimentPlanTemplates").copy(request.id, request.newId, request.name) }); }
	async experiment_plan_templates_archive(request) { return cleanJson({ archived: await this.require("labExperimentPlanTemplates").archive(request.id) }); }

	// ── labPlotRecords（绘图登记，0.4.0） ────────────────────────────────────
	async plot_records_list(request) { return cleanJson({ records: await this.require("labPlotRecords").list(request.projectId) }); }
	async plot_records_create(request) { return cleanJson({ record: await this.require("labPlotRecords").create(request) }); }
	async plot_records_update(request) { return cleanJson({ record: await this.require("labPlotRecords").update(request.id, request.patch) }); }
	async plot_records_remove(request) { return cleanJson({ removed: await this.require("labPlotRecords").remove(request.id) }); }

	/**
	 * rc.4 review（§5.2）：路线锁定已从通用 Remote/Agent 网关移出——不再提供
	 * synth_route_lock 方法。浏览器 UI 的「锁定版本」走专用 loopback
	 * user-action 端点（POST /api/lab-user-action/lock-route，同源校验 +
	 * 显式 POST + 内部调 labSynthesis.lockRoute(by:"user")）；Agent 工具注册表
	 * 从不暴露锁定。服务端 lockRoute 要求显式 actor，by 缺省/伪造一律拒绝。
	 */
	...
	/** 0.3.2：对步骤缺 SMILES 的化合物按名称 PubChem 解析并回写（仅 draft）。 */
	async synth_step_resolve_smiles(request) {
		return cleanJson({ result: await this.require("labSynthesis").resolveStepStructures(request.routeId, request.stepId) });
	}

	/** 0.4.0：Ketcher 人工补绘/修正或双源核验登记 SMILES 回写；verification 可选
	 *  （双源一致/单源登记时传入），casNumber/inchiKey 一并持久化（WP3）。 */
	async synth_step_set_structure(request) {
		return cleanJson({ route: await this.require("labSynthesis").setStepStructure(request.routeId, request.stepId, request.name, request.smiles, request.verification, { casNumber: request.casNumber, inchiKey: request.inchiKey }) });
	}
	/** 0.4.0：名称/CAS 双源结构确认；冲突只返回候选，不写入。 */
	async synth_compound_resolve_first(request) {
		return cleanJson({ result: await this.require("labSynthesis").resolveCompoundFirst(request.identifier) });
	}
	async synth_compound_resolve_dual(request) {
		return cleanJson({ result: await this.require("labSynthesis").resolveCompoundDual(request.identifier) });
	}

	/** 0.4.0：某步骤全部缺 SMILES 化合物的双源核验（只查不写，四态候选）。 */
	async synth_step_resolve_dual(request) {
		return cleanJson({ result: await this.require("labSynthesis").resolveStepCompoundsDual(request.routeId, request.stepId) });
	}
	...
	/** FR-24：Evidence 审核（0.4.0：status=corrected 时需带 correction 人工修正值）。 */
	async synth_evidence_review(request) {
		return cleanJson({ evidence: await this.require("labSynthesis").reviewEvidence(request.id, request.status, { correction: request.correction }) });
	}

	/** 0.4.0 WP4：人工完成一轮事实核验后提交审核批次。 */
	async synth_review_batch_create(request) {
		return cleanJson({ batch: await this.require("labSynthesis").createReviewBatch(request.fields ?? request) });
	}

	/** 0.4.0 WP4：读取单个审核批次（或列出某路线全部批次）。 */
	async synth_review_batch_get(request) {
		if (request.id !== undefined) return cleanJson({ batch: await this.require("labSynthesis").getReviewBatch(request.id) });
		return cleanJson({ batches: await this.require("labSynthesis").listReviewBatches(request.routeId) });
	}

	/** 0.4.0 WP4：完成/关闭审核批次。 */
	async synth_review_batch_complete(request) {
		return cleanJson({ batch: await this.require("labSynthesis").completeReviewBatch(request.id, { by: request.by ?? "user" }) });
	}

	/** 0.4.0 WP4：Agent 只处理批次内不确定项，不得覆盖人工确认/修正项；回写进入下一轮。 */
	async synth_review_uncertain_apply(request) {
		return cleanJson(await this.require("labSynthesis").applyUncertainBatch(request.id, { by: request.by ?? "agent", updates: request.updates }));
	}
	...
	async synth_plan_from_route(request) {
		...
		// 0.4.0：生成时快照当前激活的实验计划模板（版本不可变），供产物复现追溯。
		let templateSnapshot;
		try {
			const template = await this.require("labExperimentPlanTemplates").resolve("experiment-plan-default");
			if (template) {
				templateSnapshot = { id: template.id, version: template.version, name: template.name, sections: template.sections };
			}
		} catch {
			templateSnapshot = undefined; // 模板服务不可用不阻断计划生成
		}
		const plan = await chemistry.createExperimentPlan({ id: planId, routeId: route.id, ...fields, ...(templateSnapshot ? { templateSnapshot } : {}) });
	...
	async manual_capture_cancel(request) {
		const task = await this.require("labCapture").cancelTask(request.taskId, request.reason);
		return cleanJson({ task });
	}
	async manual_capture_claim_agent(request) {
		const claimed = this.require("labCapture").claimAgentCaptureTask(request.taskId);
		const { tokenSha256, ...task } = claimed.task;
		return cleanJson({ task: { ...task, token: claimed.token } });
	}
	async manual_capture_desktop_status_update(request) {
		return cleanJson({ status: this.require("labCapture").reportDesktopWebVpnStatus(request) });
	}
	async manual_capture_desktop_action_claim(request) {
		return cleanJson({ action: this.require("labCapture").claimDesktopWebVpnAction(request.projectId) ?? null });
	}
...
const REMOTE_INITIALIZERS = [
 ...["synth_compound_resolve_first","characterization_list","characterization_submit","characterization_retry","characterization_dispatch_failed"].flatMap(name=>markRemote(LabRemoteService.prototype,name)),
	...
	...markRemote(LabRemoteService.prototype, "experiment_plan_templates_list"),
	...markRemote(LabRemoteService.prototype, "experiment_plan_templates_resolve"),
	...markRemote(LabRemoteService.prototype, "experiment_plan_templates_create"),
	...markRemote(LabRemoteService.prototype, "experiment_plan_templates_update"),
	...markRemote(LabRemoteService.prototype, "experiment_plan_templates_copy"),
	...markRemote(LabRemoteService.prototype, "experiment_plan_templates_archive"),
	...markRemote(LabRemoteService.prototype, "plot_records_list"),
	...markRemote(LabRemoteService.prototype, "plot_records_create"),
	...markRemote(LabRemoteService.prototype, "plot_records_update"),
	...markRemote(LabRemoteService.prototype, "plot_records_remove"),
	...
	...markRemote(LabRemoteService.prototype, "tasks_search_delete"),
	...
	...markRemote(LabRemoteService.prototype, "tasks_report_delete"),
	...
	...markRemote(LabRemoteService.prototype, "synth_route_delete"),
	...
	// rc.4 review（§5.2）：synth_route_lock 已移出 Remote 网关（专用 user-action 端点）。
	...markRemote(LabRemoteService.prototype, "synth_route_update_step"),
	...markRemote(LabRemoteService.prototype, "synth_step_resolve_smiles"),
	...markRemote(LabRemoteService.prototype, "synth_step_resolve_dual"),
	...markRemote(LabRemoteService.prototype, "synth_step_set_structure"),
	...markRemote(LabRemoteService.prototype, "synth_compound_resolve_dual"),
	...
	...markRemote(LabRemoteService.prototype, "synth_review_batch_create"),
	...markRemote(LabRemoteService.prototype, "synth_review_batch_get"),
	...markRemote(LabRemoteService.prototype, "synth_review_batch_complete"),
	...markRemote(LabRemoteService.prototype, "synth_review_uncertain_apply"),
	...
	...markRemote(LabRemoteService.prototype, "manual_capture_cancel"),
	...markRemote(LabRemoteService.prototype, "manual_capture_claim_agent"),
	...markRemote(LabRemoteService.prototype, "manual_capture_desktop_status_update"),
	...markRemote(LabRemoteService.prototype, "manual_capture_desktop_action_claim"),
