...
export class LabRemoteService extends TypertRemoteService {
	...
	/** 0.3.2：Ketcher 人工补绘/修正某化合物 SMILES 回写（仅 draft，source=manual）。 */
	async synth_step_set_structure(request) {
		return cleanJson({ route: await this.require("labSynthesis").setStepStructure(request.routeId, request.stepId, request.name, request.smiles) });
	}
