 ...
 *
 * 返回 Promise。Windows 下 `taskkill` 是**异步 spawn**：原先 fire-and-forget 直接返回，
 * 调用方会在进程树真正死掉之前继续往下走——"已终止渲染进程"这句报错与实际状态不符，
 * 测试也只能赌一个固定 deadline。本机进程创建受 EDR 挂钩，实测从调用到子进程真正退出
 * 需约 0.9s，高负载时会超过测试原来的 3s 上限而误报。
 *
 * 调用方可 await（需要确定性）或忽略返回值（只需尽力而为），两者都支持。
 */
	...
	try {
		if (platform === "win32") {
			return new Promise((resolvePromise) => {
				let settled = false;
				// 只结算一次：error 与 exit 可能都触发，重复 resolve 无害但要避免竞态歧义。
				const finish = () => {
					if (settled) return;
					settled = true;
					resolvePromise();
	...
	try {
		...
		await new Promise((resolvePromise, rejectPromise) => {
			...
			const timer = setTimeout(() => {
				if (settled) return;
				settled = true;
				// 先真正终止进程树再拒绝：否则"已终止渲染进程"这句是假的，
				// 残留的 python/fitz 还会和下一次渲染抢 CPU 与临时目录。
				void killPythonTree(child, { platform }).finally(() => {
