...
export function apply(ctx) {
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render(args, value) {
				if (!value.ok) return `读取课题核心记忆失败：${value.error ?? "未知错误"}`;
				return [
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render(args, value) {
				if (!value.ok) return `提交课题核心记忆失败：${value.error ?? "未知错误"}`;
				return `已提交课题（${value.projectId}）核心记忆 v${value.version}${value.changeNote ? `：${value.changeNote}` : ""}。当前版本历史：${value.history.map((h) => `v${h.version}(${h.changeNote})`).join(" → ")}`;
			}
		},
		timeoutMs: 30000,
		async execute(args, exec) {
