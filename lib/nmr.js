...
export class LabNmrService extends Service {
	...
	async createDataset({ id, projectId, name, fidPath, structurePath, nucleus, solvent, compound, entityId, deuteratedSolvent, date }) {
		...
		const row = nmrDatasetSchema.parse({
			...
			compound, entityId, deuteratedSolvent, date, status: "prepared", createdAt: now, updatedAt: now
		});
