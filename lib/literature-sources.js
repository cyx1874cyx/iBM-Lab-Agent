...
export function defaultBrowserCandidates(platform = process.platform, env = process.env) {
	const candidates = [env.LIT_BROWSER_PATH];
	if (platform === "win32") {
...
export class LabLiteratureSourcesService extends Service {
	...
	async statuses({ sourceIds, force = false } = {}) {
		...
		return {
			checkedAt: now(),
			browserMode: this.resolveBrowserMode(),
			sources: statuses
		};
	}
	 ...
	 *   web-current          # 传统 DSH Web 运行在真实浏览器，复用当前标签页
	 ...
	 * 兼容旧配置：browserMode 未配置时，clientManagedBrowser=true → web-current。
	 */
	resolveBrowserMode() {
		const mode = String(this.config.browserMode ?? "").trim();
		if (["web-current", "managed-edge", "desktop-edge-handoff"].includes(mode)) return mode;
		return this.config.clientManagedBrowser ? "web-current" : "managed-edge";
	}

	/** 当前浏览器模式是否由外部 Edge（Desktop handoff）承担登录与下载。 */
	isDesktopEdgeHandoff() {
		return this.resolveBrowserMode() === "desktop-edge-handoff";
	}
	...
	async connect(sourceId, resourceUrl, { mode } = {}) {
		...
		const requestedMode = mode ?? (this.resolveBrowserMode() === "web-current" ? "current" : this.resolveBrowserMode() === "desktop-edge-handoff" ? "handoff" : "managed");
		if (requestedMode === "current") {
			...
			return {
				...
				message: "已打开入口页面。请在 Microsoft Edge 中完成人工登录。此模式复用浏览器环境，不支持后台自动捕获 PDF。"
			};
		}
		if (requestedMode === "handoff") {
			// desktop-edge-handoff：登录/下载由外部 Edge 承担，PDF/SI 经
			// capture handoff + 扩展 + Native Messaging 回传。这里只登记
			// 会话入口，实际打开由 Desktop URL Router（open_in_edge）完成。
			const updated = await this.persistSession(sourceId, {
				resourceUrl: entryUrl,
				browser: "edge-handoff",
				debuggingPort: undefined,
				state: "browser-open",
				lastOpenedAt: now(),
				lastError: undefined
			...
			return {
				...
				message: "将在 Microsoft Edge 中打开该数据库。请在可见窗口中完成登录/协议/验证码；受保护 PDF/SI 会经文献捕获扩展自动回传课题。"
			};
		}
		if (requestedMode !== "managed") throw new Error("未知的浏览器连接模式");
	...
	async verify(sourceId) {
		...
		if (session?.browser === "current-browser" || session?.browser === "edge-handoff") {
	...
	managedSession(sourceId) {
		// 仅复用具备 CDP debuggingPort 的受控浏览器会话；current-browser（Web 宿主页）
		// 与 edge-handoff（外部 Edge 人工会话）均无端口、不可 CDP 接管，显式排除。
		const direct = sourceId && this.getSession(sourceId);
		if (direct?.debuggingPort && direct.browser !== "current-browser" && direct.browser !== "edge-handoff") return direct;
		for (const key of this.table.keys()) {
			const session = this.table.get(key);
			if (session?.debuggingPort && session.browser !== "current-browser" && session.browser !== "edge-handoff") return session;
		}
		return undefined;
	}
	...
	async handleDownloadRequest(req, res) {
		...
		try {
			...
			res.writeHead(200, {
				...
				"content-disposition": `${preview ? "inline" : "attachment"}; filename*=UTF-8''${encodeURIComponent(job.fileName)}`,
