...
export function apply(ctx) {
	const executePublisherDownload = async (args, exec, { natureOnly = false } = {}) => {
		try {
			...
			const bundle = ctx.labTasks.getBundle(args.bundleId);
			...
			if (requiresVpn && !webvpn.iwanReady) {
				if (!webvpn.ready && !args.loginConfirmed) {
					...
					return {
						...
						status: "webvpn-login-required", requiresUserAction: true,
				...
				if (!webvpn.ready && args.loginConfirmed && (webvpn.stale || !webvpn.windowOpen || webvpn.state !== "waiting-login")) {
					ctx.labCapture.requestDesktopWebVpnLogin(resolved.projectId);
					return { ok: true, bundleId: bundle.id, kind: args.kind, publisher: publisher.name, status: "webvpn-login-required", requiresUserAction: true, question: "WebVPN 窗口尚未就绪，已重新在右侧打开。请完成登录后再选择“我已登录”。" };
				}
			}
			...
			return { ok: true, taskId: task.id, bundleId: bundle.id, kind: task.kind, publisher: publisher.name, status: "queued", accessMode: webvpn.iwanReady ? "iwan" : requiresVpn ? "webvpn" : "direct" };
		} catch (error) {
	...
	const publisherDownloadOutput = {
		schema: {
			...
			properties: {
				...
				bundleId: { type: "string" }, kind: { type: "string" }, publisher: { type: "string" }, status: { type: "string" }, accessMode: { type: "string" },
				requiresUserAction: { type: "boolean" }, question: { type: "string" }
			}
		},
		render(args, value) {
			if (!value.ok) return [{ type: "text", text: `出版社浏览器下载未启动：${value.error ?? "未知错误"}` }];
			if (value.requiresUserAction) return [{ type: "text", text: value.question }];
			return [{ type: "text", text: `已把 ${value.publisher || "出版社"} ${value.kind === "pdf" ? "正文" : "补充材料"}交给软件内浏览器自动下载（${value.accessMode === "iwan" ? "iWAN 直访" : value.accessMode === "webvpn" ? "WebVPN" : "公开直连"}）。taskId: ${value.taskId}` }];
		}
	};

	ctx.tools.register(defineTool({
		name: "lab_publisher_browser_download",
		description: "在软件内浏览器中自动下载已登记论文。iWAN 全部路由可用时直接访问出版社，否则按规则使用 WebVPN；支持 Nature Portfolio、SpringerLink、Science/AAAS、Elsevier、ACS、RSC、IEEE，以及仅在 iWAN 下启用的 Wiley。可处理正文 PDF 预览器二次保存以及 PDF/DOCX/ZIP 补充材料。任务创建后用 lab_publisher_browser_download_status 查询进度。",
		parameters: {
	...
	ctx.tools.register(defineTool({
		...
		description:
			"在 iBM Lab Agent 软件内浏览器中下载已登记 Nature 文献。正文使用用户已手动建立的 WebVPN 会话；SI 直连并自动识别 Supplementary/Supporting Information 或 Supplementary Methods。" +
		...
		output: {
			...
			render(args, value) {
				...
				if (value.requiresUserAction) return [{ type: "text", text: value.question }];
				return [{ type: "text", text: `已把 ${value.kind === "pdf" ? "正文 PDF" : "补充材料"}交给软件内浏览器自动下载。taskId: ${value.taskId}` }];
			}
		},
		timeoutMs: 15000,
	...
	const executePublisherDownloadStatus = async (args, exec) => {
		try {
			...
			const queuedTasks = typeof ctx.labCapture.listTasks === "function"
			...
			if (task.status === "armed") {
				if (!desktop.stale && desktop.pendingTaskId === task.id) phase = desktop.state;
				else if ((task.kind === "pdf" || !/10\.(?:1038|1007)\//i.test(task.publisherUrl || "")) && !desktop.ready && !desktop.iwanReady) phase = "waiting-login";
				else phase = "queued";
			}
			const labels = {
				queued: "任务已排队，等待软件内浏览器接管",
				"waiting-login": "正在等待 WebVPN 登录；请在软件侧栏完成登录",
				ready: "WebVPN 已可用，准备打开出版社页面",
				navigating: "正在打开出版社页面",
				"waiting-download": "正在查找并点击下载入口",
				downloading: "正在下载文件",
				uploading: "下载完成，正在归档到课题",
				completed: "下载完成，文件已归档到课题",
				failed: "下载失败",
				expired: "下载任务已过期",
				cancelled: "下载任务已取消",
				error: "软件内浏览器发生错误"
			...
			return cleanJson({
				...
				status: task.status,
				phase,
				message: `${labels[phase] ?? phase}${phase === "queued" && queuePosition ? `（队列第 ${queuePosition} 位）` : ""}${detail}`,
				queuePosition,
				fileName: task.fileName,
				size: task.size,
				downloadedBytes: desktop.pendingTaskId === task.id ? desktop.downloadedBytes : undefined,
	...
	const publisherStatusOutput = {
		...
		render(args, value) {
			return [{ type: "text", text: value.ok ? value.message : `出版社下载状态查询失败：${value.error ?? "未知错误"}` }];
		}
	};
