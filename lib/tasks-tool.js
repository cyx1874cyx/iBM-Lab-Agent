...
export function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render(args, value) {
				...
				return [{ type: "text", text: `已登记文献检索（${value.runId}，${value.status}，${value.count} 条结果）。可在课题面板「文献资料 → 文献检索」查看，.ris 按钮可下载该检索的文献题录。` }];
			}
		},
		timeoutMs: 120000,
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			shortCitation: { type: "string", description: "可选：文献短引用（面板精读条目标题，如 Zhu et al., 2024）" },
		...
		output: {
			...
			render(args, value) {
				...
				return [{ type: "text", text: `已登记精读报告（${value.reportId}，${value.status}${value.shortCitation ? `：${value.shortCitation}` : ""}）。可在课题面板「文献资料 → 文献精读」查看概览/下载报告。` }];
			}
		},
		timeoutMs: 30000,
		async execute(args, exec) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			skipAudit: { type: "boolean", description: "可选：显式跳过精读审计门禁（仅在你明确要求时使用，bundle 侧 provenance 留下『审计跳过 _ 用户要求』记录）；默认 false" }
		},
		output: {
			schema: {
				type: "object",
