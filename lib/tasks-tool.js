...
export function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			try {
				...
				if (!/^10\.1038\//i.test(String(bundle.doi || "").trim())) return { ok: false, error: "该工具当前仅支持 DOI 以 10.1038/ 开头的 Nature 文献" };
				...
				if (args.kind === "pdf") {
					...
					if (!webvpn.ready && !args.loginConfirmed) {
						...
						return {
							...
							bundleId: bundle.id,
							kind: args.kind,
							status: "webvpn-login-required",
							requiresUserAction: true,
							question: "已在右侧打开 WebVPN。请完成登录；登录成功后选择或回复“我已登录”，我再继续访问并下载正文。"
						};
					}
					if (!webvpn.ready && args.loginConfirmed && (webvpn.stale || !webvpn.windowOpen || webvpn.state !== "waiting-login")) {
						ctx.labCapture.requestDesktopWebVpnLogin(resolved.projectId);
						return { ok: true, bundleId: bundle.id, kind: args.kind, status: "webvpn-login-required", requiresUserAction: true, question: "WebVPN 窗口尚未就绪，已重新在右侧打开。请完成登录后再选择“我已登录”。" };
					}
				}
				const task = await ctx.labCapture.createAgentCaptureTask({
					...
					bundleId: bundle.id,
					kind: args.kind,
					...(args.kind === "pdf" && args.loginConfirmed === true ? { loginConfirmedByUser: true } : {})
				});
				return { ok: true, taskId: task.id, bundleId: bundle.id, kind: task.kind, status: "queued" };
			} catch (error) {
	...
	ctx.tools.register(defineTool({
		...
		description: "查询 lab_nature_browser_download 返回的 taskId 状态。直接返回排队、等待 WebVPN 登录、打开页面、下载、归档、完成或失败状态，无需读取本地存储文件。",
		parameters: {
			projectId: { type: "string", description: "可选：课题编号；缺省按当前会话绑定或工作目录反查" },
		...
		output: {
			...
			render(args, value) {
				return [{ type: "text", text: value.ok ? value.message : `Nature 下载状态查询失败：${value.error ?? "未知错误"}` }];
			}
		},
		timeoutMs: 5000,
		async execute(args, exec) {
			try {
				...
				if (task.status === "armed") {
					...
					else if (task.kind === "pdf" && !desktop.ready) phase = "waiting-login";
					else phase = "queued";
				}
				const labels = {
