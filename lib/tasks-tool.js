...
export function apply(ctx) {
	const executePublisherDownload = async (args, exec, { natureOnly = false } = {}) => {
		try {
			...
			if (publisher.paused) return { ok: false, error: "Wiley Online Library 的学校 WebVPN 访问暂不可用，本版本暂停自动下载" };
			...
			return { ok: true, taskId: task.id, bundleId: bundle.id, kind: task.kind, publisher: publisher.name, status: "queued" };
		} catch (error) {
	...
	const publisherDownloadOutput = {
		schema: {
			...
			properties: {
				...
				bundleId: { type: "string" }, kind: { type: "string" }, publisher: { type: "string" }, status: { type: "string" },
		...
		render(args, value) {
			...
			return [{ type: "text", text: `已把 ${value.publisher || "出版社"} ${value.kind === "pdf" ? "正文" : "补充材料"}交给软件内浏览器自动下载。taskId: ${value.taskId}` }];
		}
	};

	ctx.tools.register(defineTool({
		name: "lab_publisher_browser_download",
		description: "在软件内浏览器中自动下载已登记论文。支持 Nature Portfolio、SpringerLink、Science/AAAS、Elsevier、ACS、RSC 和 IEEE；按出版社规则处理正文 PDF 预览器二次保存以及 PDF/DOCX/ZIP 补充材料。Wiley 暂停。需要 WebVPN 时会先请求用户登录。任务创建后用 lab_publisher_browser_download_status 查询进度。",
		parameters: {
	...
	const executePublisherDownloadStatus = async (args, exec) => {
		try {
			...
			if (task.status === "armed") {
				...
				else if ((task.kind === "pdf" || !/10\.(?:1038|1007)\//i.test(task.publisherUrl || "")) && !desktop.ready) phase = "waiting-login";
				else phase = "queued";
			}
			const labels = {
