...
/** `soffice` is a PATH command, not an absolute renderer configuration. */
...
/**
 * On Windows `soffice.exe` is a GUI launcher and often returns no version
 * text when its stdout is piped.  The adjacent `soffice.com` is the console
 * launcher for the exact same LibreOffice installation and is reliable for
 * headless conversion and version probing.
 */
function preferredSofficeCommand(command, platform) {
	if (platform !== "win32" || !/soffice\.exe$/i.test(command)) return command;
	const consoleCommand = command.replace(/\.exe$/i, ".com");
	return existsSync(consoleCommand) ? consoleCommand : command;
}
...
function sofficeVersion(command, platform) {
	return new Promise((resolve) => {
		let child;
		try {
		child = spawn(preferredSofficeCommand(command, platform), ["--headless", "--version"], {
...
export async function resolveSofficeExecutable({ explicit, platform = process.platform } = {}) {
	// 1) 显式配置
	if (explicit && !isBareCommand(explicit)) {
		const command = preferredSofficeCommand(explicit, platform);
		const pathExists = existsSync(command);
		const pathUsable = pathExists ? await sofficeVersion(command, platform) : null;
		if (pathUsable) {
			return { command, source: "explicit", version: pathUsable, detail: command, hint: "使用显式配置的 LibreOffice 可执行文件。" };
		}
		return {
			command: null,
	...
	// A legacy preset used `soffice` as its default config.  It is not a file
	// path, so let the normal PATH lookup below handle it instead of rejecting
	// it before the standard Windows install locations are examined.
	// 2) 典型安装路径
	const candidates = [...(SOFFICE_CANDIDATES[platform] ?? [])];
	if (platform === "win32" && process.env.LOCALAPPDATA) {
		candidates.push(join(process.env.LOCALAPPDATA, "Programs", "LibreOffice", "program", "soffice.com"));
		candidates.push(join(process.env.LOCALAPPDATA, "Programs", "LibreOffice", "program", "soffice.exe"));
	}
	for (const path of candidates) {
		if (!existsSync(path)) continue;
		const command = preferredSofficeCommand(path, platform);
		const version = await sofficeVersion(command, platform);
		if (version) {
			return { command, source: "paths", version, detail: command, hint: "检测到典型安装路径中的 LibreOffice。" };
		}
	}
	// 3) PATH 探测
	for (const name of ["soffice.com", "soffice", "soffice.exe"]) {
