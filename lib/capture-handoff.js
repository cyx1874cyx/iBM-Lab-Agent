 ...
 *      http://127.0.0.1:<port>/lab/capture/<captureId>#t=<token>；
...
/** handoff 页面路由前缀（path 参数带 captureId）。 */
...
(() => {
  ...
  const armCapture = () => new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => finish(reject, new Error("未检测到 iBM 文献捕获扩展。请确认扩展已安装并启用（edge://extensions），然后在扩展弹窗中信任本机 iBM 页面后点击重试。")), 3000);
  ...
  window.addEventListener("message", (event) => {
    ...
    if (data.type === "CAPTURE_UPLOADING") { state("working"); }
    if (data.type === "CAPTURE_COMPLETED") { state("ok"); showError(""); $("meta").textContent = "文件已归档：" + ((data.payload && data.payload.fileName) || ""); }
    if (data.type === "CAPTURE_FAILED") { state("error"); showError((data.payload && data.payload.error) || "捕获失败"); }
  });
  start();
})();
</script>
...
export class CaptureHandoffService extends Service {
	...
	/** 从 /lab/capture/<captureId> 解析任务 id；不匹配返回 undefined。 */
	parseTaskId(pathname) {
		const match = /^\/lab\/capture\/([^/]+)\/?$/.exec(String(pathname ?? ""));
	...
	async handleHandoff(req, res) {
		try {
			...
			const url = new URL(req.url ?? HANDOFF_PATH, "http://localhost");
			const taskId = this.parseTaskId(url.pathname);
			if (!taskId) {
