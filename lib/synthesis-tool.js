...
export function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render(args, value) {
				...
				const diagnostics = Object.entries(row.sources || {}).map(([name, source]) => source?.error ? `${name}: ${source.error}` : null).filter(Boolean);
				return [{ type: "text", text: `${args.identifier}：${row.status}${row.smiles ? `；SMILES ${row.smiles}` : "；未得到可自动采用的结构"}${row.casNumber ? `；CAS ${row.casNumber}` : ""}${diagnostics.length ? `；来源诊断：${diagnostics.join("；")}` : ""}。` }];
			}
		},
		timeoutMs: 45000,
		async execute(args) {
	...
	// ── 字段级证据 ───────────────────────────────────────────────────────────
	ctx.tools.register(defineTool({
		name: "lab_synth_structure_candidate_add",
		description:
			"为论文中的新产物登记一个可视化结构候选并送入事实核验。method=literature-inference 表示根据底物、反应类型和文献描述推测；" +
			"method=visual-extraction 表示从 Scheme/Figure 的结构图视觉读取。两种方式都必须绑定已归档原文 bundleId 与页码，候选会立即在路线中渲染，但保持待核验；" +
			"用户确认后固化，无法确认会撤下候选，人工修正会以 Ketcher/SMILES 结果替换。不得把不确定结构伪装成数据库确认结构。",
		parameters: {
			routeId: { type: "string", required: true, description: "合成路线 id" },
			stepId: { type: "string", required: true, description: "候选所属步骤 id，如 s2" },
			name: { type: "string", required: true, description: "新产物或中间体在文献中的名称/编号" },
			smiles: { type: "string", required: true, description: "Agent 推测或视觉读取出的候选 SMILES；必须保留立体化学，不确定处不得擅自补写" },
			role: { type: "string", description: "默认 product；也可为 reactant/reagent/catalyst/unknown" },
			method: { type: "string", required: true, description: "literature-inference | visual-extraction" },
			sourceName: { type: "string", required: true, description: "原文名称，如论文题名、SI 或 Scheme 2" },
			sourceType: { type: "string", description: "可选：paper-main/paper-si/model-inference；默认按 method 选择" },
			sourceKind: { type: "string", description: "可选：pdf 或 si" },
			bundleId: { type: "string", required: true, description: "已归档正文/SI 的 bundle id，用于右侧原文核验" },
			page: { type: "string", required: true, description: "结构所在页码，如 5 或 S12" },
			figure: { type: "string", description: "可选：Scheme/Figure 编号" },
			bbox: { type: "array", items: { type: "number" }, description: "可选：结构图区域 [x1,y1,x2,y2]" },
			basis: { type: "string", required: true, description: "推测依据或视觉读取说明；写清反应位点、保留/新生键及任何不确定性" },
			doi: { type: "string", description: "可选：文献 DOI" },
			confidence: { type: "string", description: "high | medium | low | unknown；文献推测通常不得高于 medium" }
		},
		output: {
			schema: { type: "object", additionalProperties: false, properties: { ok: { type: "boolean", required: true }, error: { type: "string" }, evidence: { type: "object", additionalProperties: true }, route: { type: "object", additionalProperties: true } } },
			render(args, value) {
				if (!value.ok) return [{ type: "text", text: `结构候选登记失败：${value.error ?? "未知错误"}` }];
				return [{ type: "text", text: `已登记「${args.name}」结构候选并在路线中渲染（${args.method === "visual-extraction" ? "图片提取" : "文献推测"}；Evidence ${value.evidence.id}）。该结构仍待用户在事实核验中确认或补绘。` }];
			}
		},
		timeoutMs: 10000,
		async execute(args) {
			try {
				const result = await ctx.labSynthesis.addStructureCandidate({
					routeId: args.routeId,
					stepId: args.stepId,
					name: args.name,
					smiles: args.smiles,
					role: args.role ?? "product",
					method: args.method,
					...
					excerpt: args.basis,
					doi: args.doi,
					confidence: args.confidence ?? (args.method === "visual-extraction" ? "medium" : "low")
				});
				return { ok: true, route: cleanJson(result.route), evidence: cleanJson(result.evidence) };
			} catch (error) {
				return { ok: false, error: error.message };
			}
		}
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			page: { type: "string", description: "可选：严格 PDF 页码，只能是数字或 S+数字（如 5、S12）；Scheme/Figure 编号必须写 figure，不得混入 page" },
			figure: { type: "string", description: "可选：图号（如 Figure 2a / Scheme 1）" },
			table: { type: "string", description: "可选：表号" },
			excerpt: { type: "string", description: "可选：同一来源中的逐字原文摘录；不得加入中文解释、省略号或拼接另一页/另一文件的文字" },
