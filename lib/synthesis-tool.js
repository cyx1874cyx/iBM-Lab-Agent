 ...
 *   - lab_synth_compound_resolve_dual  登记路线前查询 PubChem/CACTUS 结构候选
 ...
 * 每个明确化合物只查询一次；合法 CAS 可独立采用，SMILES 仍严格要求
 * PubChem/CACTUS 双源一致。用户或原文已提供的字段永不被覆盖。
 */
		...
		try {
			...
			const patch = {
				...
				verification: {
					...
					notes: result?.status === "dual-confirmed" ? "PubChem/CACTUS 双源结构一致；CAS 由 PubChem 同义词校验补充" : "CAS 可独立由 PubChem 同义词校验补充；结构未达到双源自动采用条件"
				}
			};
			if (!entry.smiles && result?.status === "dual-confirmed" && result.smiles) {
				patch.smiles = result.smiles;
				if (result.inchiKey) patch.inchiKey = result.inchiKey;
				patch.source = "pubchem";
			}
			entries.set(key, { ...entry, ...patch });
		} catch (error) {
			entries.set(key, { ...entry, verification: { status: "unresolved", sources: [], checkedAt: new Date().toISOString(), notes: `自动查询失败：${error?.message ?? error}` } });
		}
	}));
...
export function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		name: "lab_synth_compound_resolve_dual",
		description:
			"在登记合成路线前，按化合物名称或 CAS 主动查询 PubChem 与 CACTUS，返回可追溯的结构候选。" +
			"上传/提取路线时，只要反应物、产物、试剂或催化剂缺少 SMILES，就应逐一调用本工具；" +
			"dual-confirmed 可写入步骤 structures，single-source/conflict/unresolved 只能保留为待人工确认，禁止猜测结构。",
		parameters: {
		...
		async execute(args) {
			try {
				...
				return { ok: true, result: cleanJson(await ctx.labSynthesis.resolveCompoundDual(identifier)) };
			} catch (error) {
				return { ok: false, error: error.message };
			}
		}
	...
	ctx.tools.register(defineTool({
		...
		description:
			...
			"从文献/文件提取路线时，在写入前应对每个缺结构的明确化合物调用 lab_synth_compound_resolve_dual，尽量补齐 SMILES、CAS 与来源；不得仅登记名称后停止。" +
		...
		async execute(args, exec) {
			try {
				...
				const enrichedSteps = await Promise.all((args.steps ?? []).map((step, index) => enrichStepCompounds(
					...
					(identifier) => ctx.labSynthesis.resolveCompoundDual(identifier)
				)));
	...
	ctx.tools.register(defineTool({
		...
		description: "给一条 draft 合成路线追加一步（reaction 必填）。写入前应对缺结构的明确化合物调用 lab_synth_compound_resolve_dual，尽量补齐 structures；状态非 draft 时拒绝。",
		parameters: {
