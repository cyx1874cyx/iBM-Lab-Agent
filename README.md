...
开一个新对话、自动选择 **课题组科研（聚前药/高分子）** agent preset，并把
当前版本核心记忆预填进输入框（发送前仍由用户确认）；再次进入课题会复用
已绑定会话。进入课题后可按三个板块查询对话产物：**文献资料**（检索汇总、
精读报告、文献 PPT）、**研究设计**（工作规划、实验方案、合成路线）和
**表征分析**（NMR/结构结果及人工审核状态）。对话界面也做了定制：会话头部
显示当前课题徽章（点击回到课题空间），输入框上方显示课题记忆提示条。
